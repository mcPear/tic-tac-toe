...
def get_q_table(q_table_empty, agent, iter_count, lr, gamma):
    ...
    play_from_state(state_x_as_first, x_as_first_iter_count, lr, gamma, q_table, agent, game_state.o)
    for s in states_x_as_second:
        play_from_state(s, x_as_second_iter_count, lr, gamma, q_table, agent, game_state.x)

    return q_table


def play_from_state(state, iter_count, lr, gamma, q_table, agent, opponent_sign):
    for i in range(iter_count):
        # print (q_table[state])
        obs = state
        game_over = False
        while not game_over:
            cell_idx, obs_ = choose_action(q_table, obs, agent, opponent_sign)
